...
±±ºPrograma  ³MT110CFM  ºAutor  ³Ricardo P Sotomayor º Data ³  11/01/16   º±±
±±ÌÍÍÍÍÍÍÍÍÍÍØÍÍÍÍÍÍÍÍÍÍÊÍÍÍÍÍÍÍÏÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÊÍÍÍÍÍÍÏÍÍÍÍÍÍÍÍÍÍÍÍÍ¹±±
±±ºDesc.     ³ PE para gravação de Observação e campos adicionais apos    º±±
±±º          ³ Aprovação,Rejeição ou Bloqueio                             º±±
...
Local _cNumSC := PARAMIXB[1] //Número da Solicitação de Compras
Local _nOpca  := PARAMIXB[2] //Contém a opção selecionada: 1 = Aprovar; 2 = Rejeitar; 3 = Bloquear
			...
			While SC1->(!Eof()) .And. SC1->(C1_FILIAL+C1_NUM)==xFilial("SC1")+_cNumSC
				...
				If RecLock("SC1")
					...
					If SC1->(FieldPos("C1_HISTMOV")) > 0
						If _nOpca == 1  //1 = Aprovar //Legenda,Situação,Responsavel,Data,Motivo SC1->C1_APROV=='B'
							cJustifica := "Solicitação de Compras Aprovada" +cEOL
						...
						ElseIf _nOpca == 2  //2 = Bloquear/Rejeitar
							If _nItem == 1
								...
								While Empty(cJustifica)
									...
									If Empty(cJustifica)
										Aviso("Atenção - MT110CFM","É Obrigatório Informar a Justificativa da Rejeição!",{"Voltar"})
									EndIF
									_lJust := .t.
								End
							...
							EndIf
							//Legenda,Situação,Responsavel,Data,Motivo
							SC1->C1_HISTMOV := U_MNTHIST(SC1->C1_HISTMOV,cJustifica,"R") //MNTHIST(cAtual,cAdd,cStatus)
						ElseIf _nOpca == 3  //3 = Bloquear   //SC1->C1_APROV
							If _nItem == 1
								While Empty(cJustifica)
									cJustifica := "SC Devolvida para analise de líder."+cEOL
									cTitulo	:= "Devolução"
									cJustifica := SC1JUST(cJustifica,cTitulo)
									If Empty(cJustifica)
										Aviso("Atenção - MT110CFM","É Obrigatório Informar a Justificativa da Devolução!",{"Voltar"})
									EndIF
								End	
